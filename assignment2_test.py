...
def diff_dir(dir1,dir2):
	...
	for f in dcmp.common_files:
		...
		elif stat1.st_mode != stat2.st_mode:
	...
	for dirc in dcmp.common_dirs:
		...
		[a,b,c,d,e] = diff_dir(dir1+'/'+dirc,dir2+'/'+dirc)
		exist_wrong = exist_wrong + a
		content_wrong = content_wrong + b
		mode_wrong = mode_wrong + c
		time_wrong = time_wrong + d		
		right = right + e

	return (exist_wrong, content_wrong, mode_wrong, time_wrong, right)
...
time.sleep(60)
print '================================================================='
[exist_wrong, content_wrong, mode_wrong, time_wrong, right]= diff_dir('./cdir','./sdir/user')
...
if pid_child == 0:
	#child process
	os.chdir('./bin')
	f = open(cwd+'/csiebox_client_stdout','w+')
	os.dup2(f.fileno(),1)
	f = open(cwd+'/csiebox_client_stderr','w+')
	os.dup2(f.fileno(),2)
	os.execl('./csiebox_client', 'client', '../config/client.cfg')
